...
class ClassificationEvaluator(object):
    ...
    def __init__(self, eval_dir):
        ...
        if not os.path.exists(self.eval_dir): os.mkdir(self.eval_dir)
    ...
    def calculate_level_performance(
        ...
        # score_dict[self.MACRO_AVERAGE] = 0
        right_total = 0
        predict_total = 0
        standard_total = 0
        ...
        for _, label_name in id_to_label_map.items():
            ...
            # score_dict[self.MACRO_AVERAGE] += score_dict[label_name]
        ...
        fscore_dict[self.MACRO_AVERAGE] = 0 \
            if (recall_dict[self.MACRO_AVERAGE] + precision_dict[self.MACRO_AVERAGE]) == 0 else \
            2 * precision_dict[self.MACRO_AVERAGE] * recall_dict[self.MACRO_AVERAGE] / \
            (recall_dict[self.MACRO_AVERAGE] + precision_dict[self.MACRO_AVERAGE])
    ...
    def evaluate(self, predicts, standard_label_names=None, standard_label_ids=None, label_map=None, 
                 threshold=0, top_k=3, is_flat=False, epoch=0, i_al=0, n_train=0, n_ensemble=0):
        ...
        line_count = 0
        if not os.path.exists(f"{self.eval_dir}/probs_{n_train}"): os.makedirs(f"{self.eval_dir}/probs_{n_train}")
        if n_ensemble is not None:
            if not os.path.exists(f"{self.eval_dir}/probs_{n_train}/Ensemble_{n_ensemble}/"): os.makedirs(f"{self.eval_dir}/probs_{n_train}/Ensemble_{n_ensemble}/")
            debug_file = open(f"{self.eval_dir}/probs_{n_train}/Ensemble_{n_ensemble}/probs_{i_al}_{epoch}.txt", "w", encoding=cDataset.CHARSET)
        else:
            debug_file = open(f"{self.eval_dir}/probs_{n_train}/probs_{i_al}_{epoch}.txt", "w", encoding=cDataset.CHARSET)

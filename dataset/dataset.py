...
class DatasetBase(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, config, args, json_files, generate_dict=False,
                 mode=ModeType.EVAL):
        """
        ...
        """
        self.config = config
        self.logger = Logger(config, args)
        self.args = args
        self._init_dict()
        self.sample_index = []
        self.sample_size = 0
        ...
        if generate_dict:
            ...
            if self.config.data.generate_dict_using_all_json_files:
                vocab_json_files += self.config.data.test_json_files
    ...
    def _save_dict(self, dict_name=None):
        ...
        if dict_name is None:
            if not os.path.exists(f"{self.args.output_dir}{self.config.data.dict_dir}"):
                os.makedirs(f"{self.args.output_dir}{self.config.data.dict_dir}")
